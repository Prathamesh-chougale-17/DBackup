...
export async function stepExecuteDump(ctx: RunnerContext) {
    ...
    // The scope deliberately ends before the upload step so no connection is
    // held open for the duration of a multi-gigabyte transfer.
    const host = createHost(resolveTransport(sourceAdapter, sourceConfig));
    try {
        ...
        try {
            ...
            if (isAll) {
                ...
                if (sourceAdapter.getDatabases) {
                    try {
                        const fetched = await sourceAdapter.getDatabases(sourceConfig, host);
            ...
            if (sourceAdapter.test) {
                try {
                    const testRes = await sourceAdapter.test(sourceConfig, host) as { success: boolean; version?: string; edition?: string };
        ...
        try {
            dumpResult = await sourceAdapter.dump(sourceConfigWithVersion, tempFile, host, (msg, level, type, details) => ctx.log(msg, level, type, details));
        } finally {
        ...
        if (ctx.metadata && (!ctx.metadata.names || ctx.metadata.names.length === 0)) {
            try {
                if (sourceAdapter.getDatabases) {
                    ...
                    const discovered = await sourceAdapter.getDatabases(sourceConfig, host);
