...
- **MSSQL**: The SSH connection test now rejects a backup path that is not an absolute path, or that carries control characters, instead of passing whatever the request sent on to the server. Remote arguments were already quoted, so this closes no known hole - it stops the check resting on that quoting alone.
