...
- **rsync**: Fixed a directory source writing one execution-log line per file - and several for a large one - by forwarding everything rsync narrates on stdout. A run's logs are stored as a single JSON string, so a source with thousands of files put thousands of lines into the database and buried the events worth reading. Progress is reported through the progress bar and summarised at the end; what rsync sends to stderr still appears.
- **rsync**: Fixed collecting a directory source failing outright with `unrecognized option '--info=progress2'`. The flag reports one progress figure for a whole transfer, but it arrived in rsync 3.1 - and two common installations are older, including openrsync, which Apple made the default `rsync` in macOS 15. It is now asked for rather than assumed; where it is unavailable the transfer reports progress per file instead.
- **rsync**: Fixed a restore to an Rsync destination failing for a handful of files with a bare `exit code 255`, and taking far longer than the other adapters. rsync has no persistent connection of its own, so every file logged in over SSH again - well over 129 logins in under a minute for a 129-file restore, which is what an SSH server's connection-rate limiting exists to stop. A run now authenticates once and multiplexes every transfer over that connection; a server that refuses multiplexing still works, one login per transfer as before.
- **ftp**: Fixed a restore to an FTP destination failing for most of its files with "User launched a task while another one is still running". Parallel transfers shared one control connection, and the FTP client answers a second concurrent operation by closing itself - so a single collision took every transfer queued behind it down too. Each parallel transfer now gets its own connection, at most as many as the connection allows.
...
- **file-backups**: Stated what the feature is for and where it stops. Directory sources exist to carry an application's config and data alongside its database in one job; because DBackup is agentless, a full run stages the tree on its own host and needs roughly twice the source size in free space, and every byte crosses the network twice. Bulk media libraries and anything needing block-level deduplication are pointed at restic and Borg instead. The landing page, README and Storage Explorer no longer advertise restoring "one file out of a 100 GB archive" - the ranged read is what matters, not the size next to it.
