...
- **connections**: Fixed deleting a connection that is still used as a directory source or by a notification template failing with a raw database error instead of naming what still depends on it.
...
- **restore**: Fixed choosing a profile in the key dialog doing nothing at all. The choice was never sent, so the retry asked the same question and got the same answer, and the dialog closed as though it had worked. A profile that does not open the backup now says so and the dialog stays up for another try.
- **restore**: Fixed a backup becoming unopenable after its recovered key was deleted from the vault again. Only the first step of the restore page ever offered the key dialog, so the failure surfaced later - at the selection's dry run - as a message with no way to answer it. Deleting a profile also drops the cached file indexes it opened, which otherwise kept listing a backup's contents for another five minutes while every restore of it failed.
